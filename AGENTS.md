...
Closed `docs/KNOWN_ISSUES.md` fragility #56: `core/campaigns.js` `buildStatusUpdateStatePorts()` (Pause/Resume/Finish tx builder) dropped state port 2 (campaign expiry block) — same root cause as fragility #51, a different tx builder missing the same carry-forward. Port 2 is never read by the escrow script itself, so the tx always confirmed; the damage was silent — `_checkCampaignExpiry` (Fix #8) degraded to the 24h wall-clock fallback for the rest of a campaign's life after its first status change. Fixed by reading `PREVSTATE(2)` via the existing `ps()`/`coins coinid:` mechanism already used for ports 1/3/4/5/6 and carrying it forward only when present — never invented. Live-verified on the 6-node harness: fresh V4 campaign, Pause → Resume → Finish in sequence, `port 2 = "8777"` (the funded expiry block) confirmed unchanged via `coins coinid:` after all three consecutive on-chain respends. V3 not separately tested (fresh harness only mints V4; reasoning for why V4 coverage suffices is in the HISTORY entry). Incidental doc/code drift found and recorded (not fixed) as new fragility #60 — `STATE(10)` value disagrees between code and `MinimaAds.md`, both script-safe. Files: `core/campaigns.js`, `dapp/app.js`, `MinimaAds.md`. Open issues: OPEN-5, fragility #58 (unchanged), fragility #60 (new, low priority, informational). Full detail: `docs/HISTORY.md §17`, session 2026-09-09 (Fragility #56).
