...
- [x] **T7 · `[Opus]` · C‑1 core — Server‑side accrual check.** In `_handleRewardRequestInner` (`channel.handler.js`), after the `MAX_AMOUNT` cap: load campaign, compute `delta = cumulative − channel.CUMULATIVE_EARNED`, reject `delta <= 0` or `delta > unit (+ε)` where `unit = REWARD_CLICK` for clicks else `REWARD_VIEW`; enforce campaign cooldown via `LAST_VOUCHER_AT`. **Done:** accrual + cooldown guard added via `getCampaign` reload, placed after the `MAX_AMOUNT` cap and before `isDuplicate` (ε = 0.000001).
...
- [x] **T9 · `[Opus]` · C‑1 — apply the same accrual guard to the publisher voucher path** (`_doGeneratePublisherVoucher` / `_maybeGeneratePublisherVoucher`) so publisher channels can't be over‑claimed either. **Done:** `_doGeneratePublisherVoucher` now verifies `pubReward <= PUBLISHER_REWARD_VIEW + ε` and enforces cooldown via `pubChannel.LAST_VOUCHER_AT`; `_replayDeferredPublisherRewardsNow` reloads the campaign and skips any deferred row whose `AMOUNT` exceeds the unit before accumulating.
- [ ] **T10 · `[Opus]` · Two‑node verification** of the three attack scenarios in §6 + confirm a normal view/click voucher still settles end‑to‑end. **Note:** T7 and T9 are implemented; T10 still requires a manual two‑node test — (a) a normal view/click voucher settles end‑to‑end, (b) a forged `cumulative = MAX_AMOUNT` REWARD_REQUEST is rejected by the server‑side accrual delta check, (c) the campaign cooldown is enforced server‑side (a second request inside the cooldown window is rejected even if the viewer forges timing).
