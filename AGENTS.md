...
Last reviewed against codebase: 2026-04-29 (Rev 18: T-CH9 FE retry loop removed)
...
29. **ESCROW_ADDRESS `trackall:true` causes locked coins in viewer wallet — UX problem, no fix yet (see bug #16).** The viewer SW registers `ESCROW_ADDRESS` with `trackall:true` to enable `scanEscrowCoins()` campaign discovery. Side-effect: ALL coins at ESCROW_ADDRESS (including the creator's escrow change coin, e.g. 9 Minima) appear in every viewer's `confirmed` balance as locked (not sendable). This also causes a "-1 Sent" wallet entry when the escrow is spent by a channel-open tx. Verified 2026-04-29 on 2-node testnet. A fix to the CHANNEL_SCRIPT `trackall` parameter was attempted and reverted — it is a red herring; the channel script coin is a secondary issue. The escrow change is the primary source of locked Minima. Fix requires architectural change to discovery mechanism (see bug #16). Do NOT attempt to fix this by changing `trackall` on the channel script.
...
| 16 | **Viewer wallet shows locked Minima — root cause: `ESCROW_ADDRESS trackall:true` for campaign discovery** | Verified 2026-04-29 on 2-node testnet. The viewer's SW registers `ESCROW_ADDRESS` with `trackall:true` so it can discover campaigns via `scanEscrowCoins()`. This causes ALL nodes (including viewer) to track every coin at `ESCROW_ADDRESS`. The creator's escrow change (e.g. 9 Minima) appears in the viewer's `confirmed` balance but NOT `sendable` (because the viewer cannot spend the creator's escrow coin), showing as "locked". Also causes a "-1 Sent" entry in the wallet when the escrow coin is spent by the channel-open tx. The amount of locked Minima equals `escrow_total − channel_maxAmount` (e.g. 9 Minima for a 10-Minima escrow with maxAmount 1). **Proposed fix:** Replace the direct address-based `scanEscrowCoins()` lookup with a read-only UTXO query that does NOT register the script as a wallet address (i.e. use `coins address:ESCROW_ADDRESS relevant:false` instead of `relevant:true`, or use a different discovery mechanism). This requires a change to `scanEscrowCoins()` in `public/service-workers/handlers/discovery.handler.js` and potentially the SW's `newscript` call for the escrow. **Architectural decision required by maintainer before implementing.** Note: the `trackall:false` fix on the CHANNEL_SCRIPT (fragility #29) was a red herring — it does not address the root cause and has been reverted. | High — UX |
...
| T-CH8 | `channel.handler.js` `handleRewardRequest()` + `checkPendingVouchers()` + `main.js` | **NEWBLOCK-driven pending voucher queue (T-CH8, 2026-04-29).** `handleRewardRequest()` now checks `coins coinid:<channelCoinId> relevant:true` before signalling `DO_REWARD_VOUCHER`. If the coin is not indexed yet, the request payload is saved to keypair under `PENDING_VOUCHER_<campaignId>_<VIEWERKEY_UPPER>`. `checkPendingVouchers()` is called on every `NEWBLOCK`; it scans all `STATUS='open'` channels, checks for a queued payload, and dispatches `DO_REWARD_VOUCHER` once the coin is visible. Replaces the 20×5 s retry loop in `buildAndExportVoucherTx()` (FE), which is removed in T-CH9. **Bug fix (2026-04-29):** keypair key must use `viewerKey.toUpperCase()` on both the write side (`handleRewardRequest`) and the read side (`checkOnePendingVoucher`). Maxima delivers `viewer_key` with lowercase `0x` prefix; H2 returns `VIEWER_KEY` with uppercase `0X` prefix — mismatched keys caused the pending entry to never be found (silent `!raw` return). |
