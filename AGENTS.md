...
Direct continuation of the #12 live-verification session, reusing the same still-live 4-node setup. Drove one more legit view from Node 4 (waited out the 5-min campaign cooldown) using Node 2's own real publisher identity — produced a genuine publisher `CHANNEL_STATE` row + real voucher on both Node 1 and Node 2. Simulated voucher loss (the one thing that can't be forced any other way) by clearing Node 2's own `LATEST_TX_HEX`/`LAST_VOUCHER_AT` via `MDS.sql` — reproduces exactly what a real crash/corrupted-storage event leaves behind. Called the real unmodified `_requestVoucherResync(campaignId, viewerKey, 'publisher')` from Node 2's console — confirmed outgoing request carried `role:'publisher'` (#7's fix). Node 1's log: `VOUCHER_SYNC_REQUEST: resending voucher...` → `voucher resent ok=true`. Node 2's `CHANNEL_STATE` afterward: `LATEST_TX_HEX` fully restored (12552 hex chars), fresh `LAST_VOUCHER_AT`, `ROLE`/`FRAME_ID` intact. #7 confirmed working end-to-end with real Maxima traffic. Files: none (verification only). Open issues: only #13 remains open (needs an actual boot-time Maxima failure, not forceable on this harness) — everything else in the 2026-09-05 audit round is now fixed and live-verified. Full detail: `docs/HISTORY.md §17`, session 2026-09-06 (live verification: audit #7).

---

### Session: 2026-09-06 (live verification: audit #12) — full real E2E proof of the frame-ownership fix, 4 real nodes

Maintainer reset all 6 harness nodes clean specifically for this. Set up 4 real nodes: Node 1 creator (real campaign, real 500 MINIMA escrow tx), Node 2 legit publisher (real custom Frame via `saveFrame`), Node 3 attacker (just its own real Maxima identity), Node 4 viewer. One deliberate seed: inserted a `FRAMES` row on Node 1 mapping Node 2's real `frame_id` → Node 2's real pk (stands in for a frame-ownership registry this codebase doesn't yet sync automatically — confirmed no such sync exists). Then Node 4 broadcast a real `MA_TRACK_VIEW` with Node 2's `frame_id` but Node 3's (attacker's) `publisherKey`/`publisherMx` — reproducing a stolen-frame_id snippet. Everything downstream ran live and unattended: real viewer channel opened, real REWARD_REQUEST paid (`CUMULATIVE_EARNED` 0→0.05), real `PUBLISHER_REWARD_NOTIFY` sent to the attacker's route, attacker's real SW responded with its own `CHANNEL_OPEN_REQUEST` claiming the stolen frame_id. Result: creator's log — `CHANNEL_OPEN_REQUEST (publisher): frame_id owned by a different publisher — dropping` — no publisher channel created. #12's async ownership check confirmed working end-to-end. Files: none (verification only). Open issues: #13 still needs a boot-time-failure repro; #7's voucher-recovery scenario still open (this 4-node setup could be reused before the harness resets again). Full detail: `docs/HISTORY.md §17`, session 2026-09-06 (live verification: audit #12).

---

### Session: 2026-09-06 (live verification: audit #10/#14) — real-DB and real-API proof of the SDK-only fixes

Discovered MetaChain cannot test #10/#14 at all (its own MiniDapp DB has no CAMPAIGNS/CHANNEL_STATE tables — its snippet talks to the real installed MinimaAds via `MDS.comms.solo()`/`comms.handler.js`, never `sdk/index.js`; confirms the scope caveat already in `docs/TESTING_SETUP.md §12`). Also found the regular MinimaAds FE loads `sdk/index.js` but `dapp/app.js` never calls `handleMdsEvent` (dead code there too) — but `window.MinimaAds.handleMdsEvent` is still reachable from the console on any page that loads it. Used Node 6 (freshly available) to test both directly against production DB/code: **#10** — inserted a real viewer + publisher `CHANNEL_STATE` row for the same campaign, confirmed the fixed query (`AND UPPER(ROLE)='VIEWER'`) returns only the viewer row while the pre-fix query returns both (ambiguous), then cleaned up. **#14** — called `window.MinimaAds.handleMdsEvent` with a real creator pk + `status:'paused'`: confirmed `alive:false` computed correctly and `CAMPAIGNS.STATUS` actually flipped to `paused`; repeated with a fake sender pk: correctly rejected, DB unchanged. Reset campaign status afterward. Files: none (verification only). Open issues: #13 still needs a live boot-time `maxima action:info` failure repro (not forceable on demand); #12's async branch and #7's recovery scenario (previous batch) also still open. Full detail: `docs/HISTORY.md §17`, session 2026-09-06 (live verification: audit #10/#14).
