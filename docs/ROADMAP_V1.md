...
| 2 | Tier 1 (offline) regression coverage grows to match `KNOWN_ISSUES.md §3` | 🟡 In progress (6 tests) | `docs/REGRESSION_TEST_PLAN.md` | 6 pure-logic tests exist (`tests/regression/*.test.js`), covering a handful of the ~20 `docs/KNOWN_ISSUES.md §3` entries whose fix is expressible as pure logic — plus the "no self-reward" rule's actual enforcement point (`validateView`/`validateClick`, not just `selectAd`) and the OPEN-4 lineage gate's traversal logic (`escrowDescendantSet`, alongside the already-tested `escrowChildCoinId`). Grows opportunistically, per that document's own rule — not all ~20 need one (many are live-node-only bugs, which is criterion #5's job, not this one). |
