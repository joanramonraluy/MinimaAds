...
Found live during a full functional regression sweep (not a pre-existing OPEN item). Clicking Pause on a V4 campaign reported full success (`[STATUS-TX] confirmed`, local DB flipped, UI showed "Paused", zero console errors) but the tx never actually landed on-chain — verified via `MDS.cmd("coins coinid:...")` ground truth on the creator's own node: the "new" coin never existed, the old one was still unspent with `STATE(7)` still `"active"`. Root cause: `core/campaigns.js`'s `buildStatusUpdateStatePorts()` never set port 16 (`foundationfeeflag`), which `ESCROW_SCRIPT_V4` reads unconditionally — the exact same bug class as fragility #49 (fixed 2026-06-09), just a third tx builder that missed it (the on-chain status-update flow was added later). `txnpost` validates nothing (fragility #53), so it reported success for a tx KissVM silently rejected. Fix: added `{port:16, value:'0'}` to the returned port array — harmless for V3 (script doesn't read it there). Verified live: redeployed, confirmed the system self-healed the corrupted local DB state via existing (unrelated, already-correct) discovery/reconciliation logic, then re-ran Pause → Resume → Finish, checking `MDS.cmd("coins coinid:...")` ground truth after each — all three now genuinely confirm on-chain (`STATE(7)` = paused/active/finished respectively, `STATE(16)="0"`), and Node 3 (viewer) correctly picks up the Pause via its own NEWBLOCK escrow scan. Files: `core/campaigns.js`. Open issues: none new — auto-settle-on-Finish payload itself not exercised this pass (no open channel left when Finish was retested); regression sweep continues, see `docs/E2E_LIVE_RUN_2026-09-07.md`. Full detail: `docs/HISTORY.md §17`, session 2026-09-07 (fragility #55).
