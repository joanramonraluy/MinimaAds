...
Last reviewed against codebase: 2026-04-29 (Rev 19: Bug #16 phase 2 — coins relevant: platform bug + fix)
...
38. **KissVM `PREVSTATE(n)` throws `ExecutionException` if port `n` is not stored in the coin — it does NOT return a default/zero.** Verified in `refs/Minima-1.0.45/src/org/minima/kissvm/Contract.java:521`: `throw new ExecutionException("PREVSTATE Missing : "+zPrev)`. This means: any `LET var=PREVSTATE(n)` statement in a script causes the entire script to FAIL at validation time if the spending coin was not created with state port `n` stored. This applies even when `var` is only used inside an `IF` branch that is never taken. **Rule: every port read by `PREVSTATE(n)` in a script MUST be written to the coin's state at creation time**, regardless of whether the spending transaction will enter the branch that uses it. For `ESCROW_SCRIPT_V2`, this means coins at `ESCROW_ADDRESS_V2` must always carry STATE(5) (PLATFORM_KEY) and STATE(6) (MAX_PUBLISHER_BUDGET) — not just the original funding tx coin, but also any intermediate coins (split coins, change coins) created by spending it. **Fixed in `dapp/app.js` `buildAndPostChannelTx()`**: split tx now sets `port:5` and `port:6` on all outputs. `platformKeyHex` and `maxPubBudget` are propagated from `handleDoChannelOpen` and `handleDoPublisherChannelOpen` → `startPublisherChannelTxs` → `buildAndPostChannelTx` → pending contexts.
...
| 2026-05-05 | Sonnet | **Split coin PREVSTATE(5/6) missing — channel-open Tx2 always failed ESCROW_SCRIPT_V2 (fixed 2026-05-05).** Root cause: `ESCROW_SCRIPT_V2` reads `PREVSTATE(5)` (PLATFORM_KEY) and `PREVSTATE(6)` (MAX_PUBLISHER_BUDGET) in unconditional `LET` statements. KissVM throws `ExecutionException("PREVSTATE Missing")` if the port is absent from the spending coin — even if the value is never used (feeflag=0 branch). The split coin created by Tx1 (`buildAndPostChannelTx`) only stored ports 1, 3, 4, 10, 11; it was missing 5 and 6. Tx2 (channel-open) spending the split coin always failed the script check at validation → channel coin never mined → voucher queue never dispatched → `LATEST_TX_HEX` stayed empty → no settlement button. Fix: `buildAndPostChannelTx` stateCmds now include `port:5 = platformKeyHex` and `port:6 = maxPubBudget`. Both values propagated from `handleDoChannelOpen` / `handleDoPublisherChannelOpen` → `startPublisherChannelTxs` → `buildAndPostChannelTx` → all pending resume contexts. Added fragility point #38. |
