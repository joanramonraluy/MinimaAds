...
function _buildSnippet(fid, pubKey, pubMx) {
  return "<div id=\"minimaads-slot\"></div>\n"
    ...
    + "  }\n"
    + "\n"
    + "  function _safePos(val) {\n"
    + "    var s = val ? String(val).trim().toLowerCase() : 'center';\n"
    + "    var allowed = {'center':1,'top':1,'bottom':1,'left':1,'right':1,'top left':1,'top right':1,'bottom left':1,'bottom right':1};\n"
    + "    return allowed[s] ? s : 'center';\n"
    + "  }\n"
    + "\n"
    + "  function _safeUrl(val) {\n"
    + "    var t = val ? String(val).trim() : '';\n"
    ...
    + "    if (!el || !ad) { return; }\n"
    + "    el.innerHTML = '';\n"
    // Audit 2026-09-05 #8: bg/fg/pos are advertiser-controlled (arrive via
    // Maxima from any campaign creator) and used raw in style.cssText below —
    // an unvalidated value is a CSS-injection / beacon vector in the *host*
    // page (this snippet runs inside the publisher's site, not MinimaAds'
    // own origin). Same validators as renderer/renderAd.js's safeColor/safePos.
    + "    var bg  = _safeColor(ad.bg_color, '#ffffff');\n"
    + "    var fg  = _safeColor(ad.text_color, '#111111');\n"
    + "    var pos = _safePos(ad.image_position);\n"
    ...
    + "        var imgCtaUrl = _safeUrl(ad.cta_url);\n"
    ...
    + "      var ctaUrl = _safeUrl(ad.cta_url);\n"
