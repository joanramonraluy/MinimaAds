...
**#5 — SDK direct-MAXIMA path applies `CAMPAIGN_PAUSE`/`CAMPAIGN_FINISH` with no creator check (AUD-3's SW fix never mirrored)** ✅ Fixed 2026-09-06 — both branches now gated by `_assertCampaignCreatorSender` (AUD-1's SDK helper) before `setCampaignStatus`. See `docs/HISTORY.md §17`, session 2026-09-06 (SDK sender-auth mirror).
...
**#6 — SDK `_persistCampaignPayload` lacks the AUD-4 identity-pinning gate → creator-identity poisoning re-enables the AUD-1 attack chain on SDK hosts** ✅ Fixed 2026-09-06 — ported the AUD-4 gate via new `_resolveStrongCampaignCreatorPk`; identity fields pinned to the existing row when the sender isn't strongly verified. See `docs/HISTORY.md §17`, session 2026-09-06 (SDK sender-auth mirror).
...
**#7 — Publisher channels have no working voucher-recovery path: `role` is dropped at every VOUCHER_SYNC hop** ✅ Fixed 2026-09-06 — added `role`/`frame_id` to all four sites (SW's REWARD_VOUCHER resend, FE's `_requestVoucherResync`, SDK's `_onReconnect` SELECT+payload) and to `MinimaAds.md §8.12`. **Verified live end-to-end 2026-09-06** — real publisher channel + voucher, simulated voucher loss (`LATEST_TX_HEX` cleared), called the real `_requestVoucherResync(..., 'publisher')`, confirmed the creator correctly resent the voucher against the publisher row and the publisher's `CHANNEL_STATE` was fully restored. See `docs/HISTORY.md §17`, sessions 2026-09-06 (audit #7/#12/#15) and (live verification: audit #7).
...
**#8 — Embedded publisher snippet re-implements the ad renderer without renderAd.js's hardening (CSS injection / beacon in the host MiniDapp)** ✅ Fixed 2026-09-06 — inlined `_safeColor`/`_safePos`/`_safeUrl` (same logic as `renderer/renderAd.js`) into the generated snippet. Confirmed live in a real third-party host (MetaChain). See `docs/HISTORY.md §17`, session 2026-09-06 (audit #8).
...
**#9 — Snippet identity regression vs fragility #35: `userAddress` is the `MAX#pk#mls` route, so local validation limits and reward history key on the wrong identity** ✅ Fixed 2026-09-06 — new `_normalizeUserAddress()` in `comms.handler.js` extracts the raw pk via `parseMaximaRoute` before use. Confirmed live against a real third-party host (MetaChain). See `docs/HISTORY.md §17`, session 2026-09-06 (audit #9).
...
**#10 — `_getMyChannel` selects by campaign only (no ROLE/viewer filter) — viewer flow can operate on the publisher row** ✅ Fixed 2026-09-06 — added `AND UPPER(ROLE) = 'VIEWER'` to the query. Verified live against production DB 2026-09-06 (inserted conflicting viewer+publisher rows, confirmed fixed query returns only the viewer one). See `docs/HISTORY.md §17`, sessions 2026-09-06 (audit #10/#13/#14) and (live verification: audit #10/#14).
...
**#11 — Custom frames store the Maxima PK as `PUBLISHER_WALLET`; the SDK opens publisher channels with it as the settlement address** ✅ Fixed 2026-09-06 — `_onFrameSubmit` resolves a real wallet address via `getaddress`; SDK's `_openNewPublisherChannel` also gained a defensive fallback for pre-fix rows. See `docs/HISTORY.md §17`, session 2026-09-06 (audit #11).
...
**#12 — Publisher `CHANNEL_OPEN_REQUEST` authentication is optional for custom frames, and `_maybeGeneratePublisherVoucher`'s "any open publisher channel" fallback can misroute rewards** ✅ Fixed 2026-09-06 — both parts closed: `publisher_mx_key` now defaults to sender + async `getFrame` ownership check against FRAMES; the `LIMIT 1` any-channel fallback removed (defers instead, like the no-channel path). **Verified live end-to-end 2026-09-06** with 4 real nodes (real campaign, real publisher frame, real attacker identity, real cross-node Maxima traffic) — attacker's `CHANNEL_OPEN_REQUEST` claiming a stolen `frame_id` was genuinely rejected (`frame_id owned by a different publisher — dropping`), no publisher channel created. See `docs/HISTORY.md §17`, sessions 2026-09-06 (audit #7/#12/#15) and (live verification: audit #12).
...
**#13 — SW bootstrap dead-ends when `maxima action:info` fails at init ("retrying in 10s" never retries)** ✅ Fixed 2026-09-06 — `NEWBLOCK` handler now re-invokes `_initAfterDb()` while `MY_MAXIMA_PK` is empty. See `docs/HISTORY.md §17`, session 2026-09-06 (audit #10/#13/#14).
...
**#14 — SDK host path drops the PONG's `status` argument — paused/finished campaigns count as alive** ✅ Fixed 2026-09-06 — passes `payload.status`, and replicates the SW's local CAMPAIGNS.STATUS sync (gated behind `_assertCampaignCreatorSender`). Verified live 2026-09-06 via `window.MinimaAds.handleMdsEvent` with a real creator pk (status synced correctly) and a fake sender pk (correctly rejected, DB unchanged). See `docs/HISTORY.md §17`, sessions 2026-09-06 (audit #10/#13/#14) and (live verification: audit #10/#14).
...
**#15 — `handlePublisherRewardNotify` trusts the sender as the campaign creator** ✅ Fixed 2026-09-06 — gated behind `_assertCampaignCreatorSender`; always routes to `campaign.CREATOR_ADDRESS` (removed the `creatorKey` param that trusted the sender). See `docs/HISTORY.md §17`, session 2026-09-06 (audit #7/#12/#15).
