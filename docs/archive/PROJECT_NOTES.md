...
> **Must-read for any Core/SW task.** Contains project intent, node identity, logging conventions, runtime topology, and DB schema.
> Reference material (protocol matrix, signal contract, source-of-truth rules) lives in `docs/PROJECT_NOTES_REFERENCE.md` — consult as needed.
