...
|---|---|---|
| CH-5 | `sdk/index.js`, `core/channels.js`, `channel.handler.js`, `dapp/app.js` | **`sendable:0` — viewer reward (and creator refund) confirmed on-chain but unspendable.** Root cause: `buildAndExportVoucherTx` derived the viewer's settlement output address via `newscript "RETURN SIGNEDBY(viewerKey)"`. Coins at custom script addresses appear in `confirmed` balance but NOT in `sendable` — Minima `send` only recognizes standard wallet addresses. Fix (2026-04-28): viewer calls `getaddress` before opening a channel, stores the result as `VIEWER_WALLET_ADDR` in `CHANNEL_STATE`, and includes it in `CHANNEL_OPEN_REQUEST`. Creator's `handleDoRewardVoucher` reads `channel.VIEWER_WALLET_ADDR` directly instead of calling `deriveScriptAddress`. Settlement output[0] (viewer reward) now lands at a real wallet address → `sendable` equal to `confirmed`. The creator's refund (output[1]) still uses `deriveScriptAddress(creatorWalletPK)` — a separate improvement. |
