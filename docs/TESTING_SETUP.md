...
- Publisher flow: registering a Frame (`#frames`), embedding it, and
  confirming publisher-side rewards route correctly. **Partially resolved**
  2026-09-06 — see §12: MetaChain (a real third-party MiniDapp, not part of
  this repo) is available on this harness with a built-in "paste a MinimaAds
  snippet and run it" panel, and has been used to find and fix live bugs
  (audit #8, #9). Scope caveat in §12 before relying on it for anything else.
...
## 12) Testing with a real third-party host: MetaChain's snippet panel

Discovered/used 2026-09-06 (audit #8, #9). MetaChain is a separate, real
MiniDapp (not part of this repo) installed on Node 3 alongside our own
SW-integrated MinimaAds install (same physical node, different MDS `uid`,
different tab). It ships a **built-in panel purpose-made for pasting and
running a MinimaAds publisher snippet**, which makes it the only way on this
harness to exercise the snippet's actual runtime behavior in a genuine
third-party host page (as opposed to reading `dapp/views/frames.js`'s
generator code and reasoning about it).

**Scope caveat — read before using this for anything else**: the snippet
generated by `dapp/views/frames.js` `_buildSnippet` is fully self-contained
inline JS that talks to the local SW purely via `MDS.comms.broadcast`
(`MA_GET_AD`/`MA_TRACK_VIEW`/`MA_TRACK_CLICK`, handled by
`comms.handler.js`). It does **not** load or use `sdk/index.js` at all. So
MetaChain is useful for live-testing anything reachable through
`comms.handler.js`/`channel.handler.js` (confirmed so far: audit #8 CSS
injection, audit #9 snippet identity) but **cannot** exercise `sdk/index.js`'s
own code paths (`_getMyChannel`, `handleMdsEvent`, `_onCreatorLivenessPong`,
etc.) — audit findings #5, #6, #10, #14 remain not-live-testable on this
harness; don't spend time trying to reach them via MetaChain.

### 12.1 Getting a fresh snippet after a code change

1. Make the code change in `dapp/views/frames.js` (the snippet generator).
2. Redeploy: Node Manager → Build Pipeline → clean stray files
   (`rm -rf .playwright-mcp latest-deploy.mds`, per §4's gotcha) → "Zip &
   Install to Nodes". This closes/reloads any open MinimaAds tabs — expected.
3. Reopen MinimaAds on a node that already has a custom Frame (e.g. Node 2,
   Publisher role, `#frames`) via the dapp icon in the MDS "app store" tab.
4. Expand the Frame's "Snippet" `<details>` and extract the text
   programmatically rather than relying on the "Copy" button + a real
   clipboard round-trip (which `browser_evaluate` can't reliably read back):
   ```js
   // browser_evaluate, run on the #frames page:
   () => {
     var details = document.querySelectorAll('details');
     var target = null, count = 0;
     details.forEach(function(d){
       var summary = d.querySelector('summary');
       if (summary && summary.textContent.indexOf('Snippet') >= 0) {
         count++;
         // count === Nth Frame card on the page, in DOM order
         if (count === 2) { target = d.querySelector('pre, textarea, code'); }
       }
     });
     return target ? target.textContent : 'not found';
   }
   ```
   This returns the exact `<div id="minimaads-slot"></div><script>...</script>`
   string, including the frame's `frameId`/`publisherKey`/`publisherMx`
   baked in — copy it verbatim into the next step.

### 12.2 Pasting it into MetaChain

1. Navigate to MetaChain's tab (a separate MDS `uid` on the target node —
   list open tabs to find it, or open it from that node's MDS app-store page
   like any other MiniDapp).
2. Help → the "MinimaAds" accordion section. It has a textarea (placeholder
   "Paste MinimaAds snippet here…") plus "Run"/"Clear" buttons and a
   "● Banner active this session" status line. Per its own copy: *"The
   snippet is saved for convenience but never runs automatically on a new
   session — you control when it activates."*
3. `browser_type`/`fill` on that textarea does not reliably survive a
   React-controlled input's internal state in this app — use the native
   setter + dispatch pattern instead (same trick as writing into any
   React-controlled `<textarea>`/`<input>` via `browser_evaluate`):
   ```js
   // browser_evaluate, element = the textarea:
   (el) => {
     var snippet = "...."; // the full text from §12.1, with real quotes/newlines escaped for this JS string
     var setter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value').set;
     setter.call(el, snippet);
     el.dispatchEvent(new Event('input', {bubbles:true}));
     el.dispatchEvent(new Event('change', {bubbles:true}));
     return el.value.length; // sanity-check the paste landed
   }
   ```
4. Click "Run". Console immediately shows the snippet's own lifecycle logs
   (`[MA] snippet loaded...`, `[MA-PUBLISHER] ADDRESS FORMAT: ...`,
   `[MA] MDSCOMMS: MA_GET_AD` → `MA_AD_RESPONSE` → `MA_TRACK_VIEW` →
   `MA_TRACK_RESULT`) — check `browser_console_messages` for `confirmed:true`
