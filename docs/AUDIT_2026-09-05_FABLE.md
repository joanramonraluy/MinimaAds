...
**#7 — Publisher channels have no working voucher-recovery path: `role` is dropped at every VOUCHER_SYNC hop** ✅ Fixed 2026-09-06 — added `role`/`frame_id` to all four sites (SW's REWARD_VOUCHER resend, FE's `_requestVoucherResync`, SDK's `_onReconnect` SELECT+payload) and to `MinimaAds.md §8.12`. See `docs/HISTORY.md §17`, session 2026-09-06 (audit #7/#12/#15).
