...
- **Publisher settlement auto-settles on campaign finish, same as viewer channels**: `dapp/app.js` `_autoSettleOpenChannels` runs identically for `ROLE='publisher'` and `ROLE='viewer'` rows — both are driven by the creator's `CAMPAIGN_UPDATED{settling:true}` signal (§6.7) and settle through the same `_runSettlement` mechanics (role-agnostic: import → co-sign → post). A publisher can still settle manually anytime via `#earnings` → Settle; auto-settle just means they no longer have to remember to. **History**: audit 2026-07-18 Fix #12 originally excluded publisher rows here because `PUBLISHER_REWARD_NOTIFY` delivery reliability was unconfirmed. Re-included 2026-09-08 after OPEN-3 extended the authenticated `CAMPAIGN_FINISH` propagation to publisher rows too and a live re-test confirmed `PUBLISHER_REWARD_NOTIFY` delivery is reliable (3/3 real sends, ~25s each — see `docs/HISTORY.md §17`, session 2026-09-08).
