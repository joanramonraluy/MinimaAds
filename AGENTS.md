...
29. **`trackall:true` on the channel script causes locked coins in the viewer's wallet.** The CHANNEL_SCRIPT (`IF @COINAGE GT (40*1728) AND SIGNEDBY(PREVSTATE(1)) THEN RETURN TRUE ENDIF RETURN MULTISIG(2 PREVSTATE(1) PREVSTATE(2))`) must be registered with **`trackall:false`** (in `creator.js` `resolveChannelScriptAddress()`). With `trackall:true`, ALL nodes — including viewers — track the channel coin (1 Minima MULTISIG); it appears in their `confirmed` but NOT `sendable` balance, showing as "locked" for every open campaign. Fix: `trackall:false` means only the creator's node tracks it (sufficient for building voucher txs via `txninput coinid:...`). The viewer never sees the coin in their balance. Campaign discovery is unaffected — it uses the ESCROW_ADDRESS (a different script that correctly stays `trackall:true`).
