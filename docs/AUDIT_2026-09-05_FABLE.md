...
**#10 — `_getMyChannel` selects by campaign only (no ROLE/viewer filter) — viewer flow can operate on the publisher row** ✅ Fixed 2026-09-06 — added `AND UPPER(ROLE) = 'VIEWER'` to the query. See `docs/HISTORY.md §17`, session 2026-09-06 (audit #10/#13/#14).
...
**#14 — SDK host path drops the PONG's `status` argument — paused/finished campaigns count as alive** ✅ Fixed 2026-09-06 — passes `payload.status`, and replicates the SW's local CAMPAIGNS.STATUS sync (gated behind `_assertCampaignCreatorSender`). See `docs/HISTORY.md §17`, session 2026-09-06 (audit #10/#13/#14).
