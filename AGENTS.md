...
#10: SDK's `_getMyChannel` selected by campaign only, no `ROLE` filter — a node with both a viewer and publisher `CHANNEL_STATE` row for the same campaign (custom-frame host scenario) could pick the wrong one, silently dropping the viewer's reward. Fixed by adding `AND UPPER(ROLE) = 'VIEWER'` (schema guarantees non-null `ROLE`). #13: SW's `_initAfterDb` logged "retrying in 10s" on a failed `maxima action:info` but never actually retried — a zombie node with empty `MY_MAXIMA_PK` forever. Fixed by re-invoking `_initAfterDb()` from `NEWBLOCK` while `MY_MAXIMA_PK` is empty. #14: SDK's direct-MAXIMA path dropped the liveness PONG's `status` arg (always `undefined` → always "alive", even for finished campaigns), and never synced local `CAMPAIGNS.STATUS` like the SW does. Fixed both: pass `payload.status`, replicate the SW's status-sync (gated behind existing `_assertCampaignCreatorSender`). Files: `sdk/index.js`, `service.js`. Open issues: #10/#14 later live-verified (see entry above); #13 still needs a boot-time repro. Full detail: `docs/HISTORY.md §17`, session 2026-09-06 (audit #10/#13/#14).
