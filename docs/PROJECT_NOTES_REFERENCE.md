...
| `CREATOR_LIVENESS_PING` | Viewer SDK → Creator SW (unicast, `poll:false`) | `campaign.handler.js` | None (read-only liveness check) | — (sends `CREATOR_LIVENESS_PONG`) | Viewer tests if creator is online. Creator must respond with PONG within 3s or viewer marks campaign inaccessible (cached 2 min). Uses `viewer_mx` from PING so PONG delivery succeeds even if creator doesn't have viewer in contacts. |
| `CREATOR_LIVENESS_PONG` | Creator SW → Viewer SDK (unicast, `poll:false`) | `campaign.handler.js` | None | `CREATOR_LIVENESS_PONG` (internal) | Signals that creator is online and can process reward requests. Sent immediately on PING receipt. |
