...
### Session: 2026-09-12 (TIER1-GROWTH) — Two new Tier 1 regression tests: validation.js self-reward/limits, escrowDescendantSet

Grew `docs/REGRESSION_TEST_PLAN.md` Tier 1 from 4 to 6 tests, agent-selected per the maintainer's "continue wherever you want" go-ahead. `validation.test.js` covers `core/validation.js` `validateView()`/`validateClick()` — the actual enforcement point for the "no self-reward" rule (`CLAUDE.md §6`) and VAL-1's per-campaign cooldown, stubbing `getCampaign`/`sqlQuery` by reassigning them on the loaded vm context after load (a plain stub passed into `loadCore`'s initial globals gets clobbered by `minima.js`'s own real `sqlQuery` declaration). `escrowDescendantSet.test.js` covers the breadth-first traversal half of the OPEN-4 lineage gate (depth-1/2 node counts, `maxDepth` clamping, malformed-anchor short-circuit) — found and fixed two authoring bugs along the way: the `MDS.cmd` stub must keep the `0x` prefix on `response.hash` (the real function strips one), and comparing an empty object from inside the vm's realm needs `Object.keys().length`, not `assert.deepEqual`, since cross-realm objects fail prototype-identity checks. Files: `tests/regression/validation.test.js` (new), `tests/regression/escrowDescendantSet.test.js` (new), `docs/REGRESSION_TEST_PLAN.md`, `docs/ROADMAP_V1.md`. Open issues: none — `node tests/regression/run-all.js` → `6/6 passed`. Full detail: `docs/HISTORY.md §17`, session 2026-09-12 (TIER1-GROWTH).
