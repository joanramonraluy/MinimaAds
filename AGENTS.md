...
### Session: 2026-09-12 (OPEN-6) — H2 schema versioning + destructive migrations: **DESIGN ONLY, docs only**, pending review

Design pass on OPEN-6 (additive-only schema evolution, no destructive path, no version tracking). **No code written, no schema changed, no `MinimaAds.md` or `AGENTS.md` H2-rules edit** — proposal awaiting review, OPEN-6 stays open. Headline finding, measured against the project's own bundled `h2-2.1.214.jar` using Minima's exact connection settings (`MODE=MySQL`, autoCommit — read from `SqlDB.java:66`, not assumed): **H2 2.1.214 already provides natively idempotent destructive DDL** — `ALTER TABLE IF EXISTS t ALTER COLUMN IF EXISTS c SET DATA TYPE T` / `… RENAME TO new` / `DROP COLUMN IF EXISTS c` all apply when present, silently no-op when absent, and are safe to re-run on every boot and on fresh installs. (The MySQL spellings `MODIFY COLUMN IF EXISTS`, `CHANGE COLUMN IF EXISTS`, `RENAME COLUMN IF EXISTS` and `RENAME TABLE` are all **syntax errors** — only the `ALTER COLUMN IF EXISTS` form works.) A failed type-change was verified to be a clean no-op: no leftover `_COPY_` table, data/PK/indexes intact. So the destructive case needs **no version table** — the proposed `SCHEMA_MIGRATIONS` table is justified *only* for non-idempotent data backfills and multi-step table rewrites, and explicitly **not** as a boot-time optimisation. Migrations become a data array + runner in a new `core/schema.js`, using the already-established SW↔FE shared-code channel (`MDS.load` / `<script src>`), so both runtimes run one identical ordered set. Two verified context facts reshape the problem: **SW and FE share one H2 file** (keyed by MiniDapp UID, Java-serialised), so "apply in both runtimes" is about boot-order independence, not two schemas; and there are **no usable transactions** (DDL auto-commits; `rollback()` verified not to undo a `CREATE TABLE`). Worked example: `FRAMES.PUBLISHER_MX VARCHAR(512)` → `1024` (the other two Maxima-route columns are already 1024). Existing `ADD COLUMN IF NOT EXISTS` pattern explicitly unchanged. Files: `docs/HISTORY.md` (design entry), `AGENTS.md` (this pointer), `docs/KNOWN_ISSUES.md` (OPEN-6 status note). No `docs/TASKS.md` row added — that file tracks only sequenced `T-*` tasks and no OPEN-N item has ever had one. Open issues: **six questions, none blocking** (ship now vs on demand, include the version table at all, measure a real Maxima route before widening, fold the 30-statement `db-init.js` pyramid into the array, should a failed migration block boot, when to add the H2 matrix to `AGENTS.md`). Three side findings recorded but **not ticketed** (maintainer to decide): the FE mirrors only 5 of 12 tables; `CHANNEL_STATE.SPLIT_COINID` exists in the SW but not the FE — a live instance of the drift this design guards against; and `MinimaAds.md §3.5` has drifted from the shipped schema in three places. Full detail: `docs/HISTORY.md §17`, session 2026-09-12.
